...
class ClassificationTask(SingleOutputTask):
  ...
  def get_prediction_module(self, bert_model, features, is_training,
    ...
    # IBO
    if self.name + "_eid" in features:
        eid = features[self.name + "_eid"]
    else:
        eid = feature_spec.FeatureSpec(self.name + "_eid", [])
...
# IBO 
class WeightedClassificationTask(ClassificationTask):
  __metaclass__ = abc.ABCMeta

  def __init__(self, config: configure_finetuning.FinetuningConfig, name,
               tokenizer, label_list, weights):
    super(WeightedClassificationTask, self).__init__(config, name, tokenizer, label_list)
  ...
  def get_prediction_module(self, bert_model, features, is_training,
    ...
    losses = -tf.reduce_sum(labels * log_probs * self.weights, axis=-1)
...
class BioYesNo(ClassificationTask):
  """BioASQ Yes/No question answering."""
  def __init__(self, config: configure_finetuning.FinetuningConfig, tokenizer):
    super(BioYesNo, self).__init__(config, "yesno", tokenizer, ["0", "1"])

  def _create_examples(self, lines, split):
    examples = []
    examples += self._load_glue(lines, split, 1, 2, 0, True)
    return examples

  def get_scorer(self):
    return classification_metrics.YesnoF1Scorer()

class BioYNN(ClassificationTask):
  """BioASQ Yes/No/Neutral question answering."""
  def __init__(self, config: configure_finetuning.FinetuningConfig, tokenizer):
    super(BioYNN, self).__init__(config, "ynn", tokenizer, ["0", "1", "2"])
...
class BioYNNSubSampled(ClassificationTask):
  """BioASQ Yes/No/Neutral question answering."""
  def __init__(self, config: configure_finetuning.FinetuningConfig, tokenizer):
    super(BioYNNSubSampled, self).__init__(config, "ynnss", tokenizer, ["0", "1", "2"])
...
class BioYesNoSnippet(ClassificationTask):
  """BioASQ Yes/No question answering using snippets instead of ideal answer."""
  def __init__(self, config: configure_finetuning.FinetuningConfig, tokenizer):
    super(BioYesNoSnippet, self).__init__(config, "yesno-snippet", tokenizer, ["0", "1"])
...
class BioREGAD(ClassificationTask):
  """Gene-disease relation extraction for GAD dataset."""
  def __init__(self, config: configure_finetuning.FinetuningConfig, tokenizer):
    super(BioREGAD, self).__init__(config, "gad", tokenizer, ["0", "1"])
...
class BioREEUADR(ClassificationTask):
  """Gene-disease relation extraction EU-ADR dataset."""
  def __init__(self, config: configure_finetuning.FinetuningConfig, tokenizer):
    super(BioREEUADR, self).__init__(config, "euadr", tokenizer, ["0", "1"])
...
class BioREChemProt(ClassificationTask):
  """Protein-chemical relation extraction for Chemprot dataset."""
  def __init__(self, config: configure_finetuning.FinetuningConfig, tokenizer):
    super(BioREChemProt, self).__init__(config, "chemprot", tokenizer, ["CPR:3", "CPR:4", "CPR:5", "CPR:6", "CPR:9",  "False"])
  ...
  def get_scorer(self):
    return classification_metrics.ChemprotF1Scorer()


class BioRESparc(ClassificationTask):
  """Connectivity relation extraction for SPARC dataset."""
  def __init__(self, config: configure_finetuning.FinetuningConfig, tokenizer):
    super(BioRESparc, self).__init__(config, "sparc", tokenizer, ["0", "1"])

  def _create_examples(self, lines, split):
    examples = []
    print('_create_examples split={}'.format(split))
    examples += self._load_glue(lines, split, 0, None, 1, skip_first_line=True)
    return examples
...
class BioRESparcMulti(ClassificationTask):
  """Connectivity relation extraction for SPARC dataset (multi relation)"""
  def __init__(self, config: configure_finetuning.FinetuningConfig, tokenizer):
    super(BioRESparcMulti, self).__init__(config, "sparc-multi", tokenizer, ["AC", "FC", "False"])
  ...
  def get_scorer(self):
    return classification_metrics.SparcMultiF1Scorer()


class Attrition(ClassificationTask):
  """Attrition vs exclusion criteria classification"""
  def __init__(self, config: configure_finetuning.FinetuningConfig, tokenizer):
    super(Attrition, self).__init__(config, "attrition", tokenizer, ["0", "1"])
...
class BioAnswerFinderReranker(ClassificationTask):
  """Bio-AnswerFinder Reranking binary question answer sentence classifier"""
  def __init__(self, config: configure_finetuning.FinetuningConfig, tokenizer):
    super(BioAnswerFinderReranker, self).__init__(config, "reranker", tokenizer, ["0", "1"])
...
class SynergyReranker(ClassificationTask):
  """Bio-AnswerFinder (+Synergy data) Reranking binary question answer sentence classifier"""
  def __init__(self, config: configure_finetuning.FinetuningConfig, tokenizer):
    super(SynergyReranker, self).__init__(config, "synergy-reranker", tokenizer, ["0", "1"])
...
class BioAnswerFinderWeightedReranker(WeightedClassificationTask):
  """Bio-AnswerFinder weighted Reranking binary question answer sentence classifier"""
  def __init__(self, config: configure_finetuning.FinetuningConfig, tokenizer, weights):
    super(BioAnswerFinderWeightedReranker, self).__init__(config, "weighted-reranker", tokenizer, ["0", "1"], weights)
