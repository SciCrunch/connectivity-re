...
class SpanBasedQAScorer(scorer.Scorer):
  ...
  def _get_results(self):
    ...
    elif self._name == "bioasq":
      return sorted(squad_official_eval_v1.main(
          self._config, self._split, bioasq=True, task_name="bioasq").items())
    elif self._name == "bioasq2":
      return sorted(squad_official_eval_v1.main(
          self._config, self._split, bioasq=True, task_name="bioasq2").items())
    else:
  ...
  def write_predictions(self):
    ...
    for example in self._eval_examples:
      example_id = example.qas_id if "squad" or "bioasq" in self._name else example.qid
    ...
    # IBO
    all_nbest_json2 = collections.OrderedDict(all_nbest_json)
    for _, nbest_results in all_nbest_json2.items():
        for entry in nbest_results:
            entry["probability"] = str(entry["probability"])
            entry["start_logit"] = str(entry["start_logit"])
            entry["end_logit"]   = str(entry["end_logit"])

    utils.write_json(all_nbest_json2,
            self._config.qa_preds_nbest_file(self._name))
