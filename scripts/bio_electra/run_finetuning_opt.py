...
def create_obj_fun(config: configure_finetuning.FinetuningConfig):
...
def main():
  ...
  config = configure_finetuning.FinetuningConfig(args.model_name, args.data_dir, **hparams)
  objective = create_obj_fun(config)
  trials = Trials()
  # best = fmin(objective,
  #            space={'num_epochs': hp.choice('num_epochs',[3, 5, 10, 15, 20]),
  ...
  # best = fmin(objective,
  #            space={'num_epochs': hp.choice('num_epochs',[3, 5, 10]),
  #                   'lr': hp.choice('lr',[1e-5, 1e-4, 5e-4]),
  #                   'batch_size': hp.choice('batch_size', [12])
  #                  },
  #            algo=tpe.suggest, max_evals=9, trials=trials)
  #best = fmin(objective,
  #            space={'num_epochs': hp.choice('num_epochs',[3, 5, 10, 15, 20]),
  #                   'lr': hp.choice('lr',[1e-5, 5e-5, 1e-4, 5e-4]),
  ...
  best = fmin(objective,
              space={'num_epochs': hp.choice('num_epochs',[3, 5, 10, 15]),
                     'lr': hp.choice('lr',[1e-5, 5e-5, 1e-4, 5e-4]),
                     'batch_size': hp.choice('batch_size', [16])
                    },
              algo=tpe.suggest, max_evals=16, trials=trials)
