...
def add_scores(test_pickle_file, test_tsv_file, pred_tsv_file):
    ...
    with open(test_tsv_file, "r") as f:
        ...
        for i, row in enumerate(reader):
            ...
            line = [row[0], probs[0], probs[1], probs[2]]
            lines.append(line)

    with open(pred_tsv_file, 'w') as f:
        writer = csv.writer(f, delimiter='\t')
        writer.writerow(['sentence','ac_prob', 'fc_prob', 'no_rel_prob'])
        for line in lines:
...
if __name__ == '__main__':
    ...
    in_tsv_file = root_dir + 'finetuning_data/sparc-multi/test.tsv'
    predictions_file = '/tmp/sparc_predictions.txt'
    in_tsv_file = args.i
    predictions_file = args.o
    add_scores(root_dir + 'models/bio_electra_base_1_2M/test_predictions/sparc-multi_test_1_predictions.pkl',
